...
# Make sure MoviePy finds ffmpeg on hosted envs
...
def split_story_into_scenes(story: str, max_chars=140) -> List[str]:
    ...
    if buf:
        scenes.append(buf)
    return scenes[:10]  # cap for Shorts
...
def generate_image(prompt: str, provider: str | None = None):
    ...
    if not api_key:
        # Fallback to local-style image if no key
...
def tts_to_mp3_path(text: str, lang: str) -> str:
    """Create TTS MP3 file and return its path."""
...
# ---------- Utilities ----------
def wrap_caption(text: str, width=22) -> str:
    return "\n".join(textwrap.wrap(text, width))


def add_text_overlay(pil_img: Image.Image, caption: str) -> Image.Image:
    img = pil_img.copy()
    draw = ImageDraw.Draw(img)
    try:
...
def build_video_from_story(
    ...
    try:
        ...
        # Build visual clips (convert PIL -> numpy when creating ImageClip)
        clips = []
        for s in scenes:
            ...
            clip = ImageClip(np.array(pil)).set_duration(per_scene)
            clip = clip.resize(lambda t: 1 + 0.03 * t).set_position("center")
            clips.append(clip)

        video = concatenate_videoclips(clips, method="compose")

        # Voiceover using MoviePy only
        ...
        # Background music (if any) — loop to fill duration
    ...
    try:
        for idx, ch in enumerate(chapters, start=1):
            ...
            res = build_video_from_story(
                story_text=story, deity=deity, voice_lang=voice_lang,
        ...
        return {
            "ok": True,
            "project_dir": os.path.basename(out_dir),
            "episodes": results,
            "playlist_text": "\n".join(playlist_lines),
        }
    except Exception as e:
